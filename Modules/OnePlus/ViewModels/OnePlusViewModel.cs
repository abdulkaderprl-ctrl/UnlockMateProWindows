...
{
    public class OnePlusViewModel : ViewModelBase
    {
        ...
        public OnePlusViewModel(IAdbService adbService, IFastbootService fastbootService, ILoggerService logger, INotificationService notificationService)
        {
            ...
            BrowseFirmwareCommand = new RelayCommand(() => FirmwarePath = OpenFileDialog("Firmware files (*.zip;*.tar;*.bat;*.sh)|*.zip;*.tar;*.bat;*.sh|All files (*.*)|*.*"));

            Log("OnePlus Professional Module Initialized.");
        }
        ...
        private void Log(string message)
        {
            ...
            _logger.LogInfo($"[ONEPLUS] {message}");
        }

        private async Task DetectDeviceAsync()
        {
            IsBusy = true;
            StatusText = "Detecting OnePlus device...";
            Progress = 30;
            ...
            if (fastbootDevices.Any())
            {
                ...
                _detectedDeviceInfo = new DeviceInfo { Model = "OnePlus Fastboot", Mode = "Fastboot Mode", Serial = fb.SerialNumber };
            ...
            else if (adbDevices.Any())
            {
                ...
                {
                    _detectedDeviceInfo = new DeviceInfo { Model = "OnePlus Recovery", Mode = "Recovery Mode", Serial = adb.SerialNumber };
                    Log($"Found Recovery Device: {adb.SerialNumber}");
                }
                else if (adb.DeviceState == "sideload")
                {
                    _detectedDeviceInfo = new DeviceInfo { Model = "OnePlus Sideload", Mode = "Sideload Mode", Serial = adb.SerialNumber };
        ...
        private async Task ReadInfoAsync()
        {
            ...
            Progress = 20;
            Log("Reading OnePlus Device Information...");
            ...
            else if (_detectedDeviceInfo?.Mode == "Fastboot Mode")
            {
                ...
                var (s4, currSlot) = await _fastbootService.ExecuteFastbootCommandAsync("getvar current-slot", _detectedDeviceInfo.Serial);
                
                Log($"Product: {ExtractGetVar(product)}");
                Log($"Bootloader Unlocked: {ExtractGetVar(unlocked)}");
                Log($"Secure Boot: {ExtractGetVar(secure)}");
                Log($"Current Slot: {ExtractGetVar(currSlot)}");
                Progress = 100;
            }
            else
            {
                Log("No device detected. Please click 'Detect Device' first.");
                _notificationService.ShowNotification("OnePlus", "Please detect device first.", NotificationType.Warning);
            }
        ...
        private async Task FlashPersistAsync() => await FlashImageAsync("persist", PersistPath);
        ...
        private async Task FlashFirmwareAsync()
        {
            ...
            Log("Starting OnePlus Firmware Flash...");
            ...
            await Task.Delay(1000);
            Log("Warning: Flashing a full firmware can take several minutes.");
            ...
            Progress = 100;
            Log("All Firmware partitions flashed successfully.");
            await _fastbootService.RebootFastbootAsync(_detectedDeviceInfo.Serial, "");
