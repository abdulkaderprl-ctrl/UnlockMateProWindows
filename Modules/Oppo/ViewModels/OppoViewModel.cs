...
{
    public class OppoViewModel : ViewModelBase
    {
        ...
        public string OfpPath { get => _OfpPath; set => SetProperty(ref _OfpPath, value); }
        ...
        public ICommand FlashOfpCommand { get; }
        ...
        public ICommand BrowseOfpCommand { get; }
        public ICommand BrowsePersistCommand { get; }

        public OppoViewModel(IAdbService adbService, IFastbootService fastbootService, ILoggerService logger, INotificationService notificationService)
        {
            ...
            FlashOfpCommand = new AsyncRelayCommand(FlashOfpAsync, () => !IsBusy);
            ...
            BrowseOfpCommand = new RelayCommand(() => OfpPath = OpenFileDialog("OFP Firmware (*.ofp)|*.ofp|All Files (*.*)|*.*"));
            BrowsePersistCommand = new RelayCommand(() => PersistPath = OpenFileDialog("Persist Image (*.img)|*.img"));

            Log("Oppo Professional Module Initialized.");
        }
        ...
        private void Log(string message)
        {
            ...
            _logger.LogInfo($"[Oppo] {message}");
        }

        private async Task DetectDeviceAsync()
        {
            IsBusy = true;
            StatusText = "Detecting Oppo device...";
            Progress = 30;
            ...
            if (fastbootDevices.Any())
            {
                ...
                _detectedDeviceInfo = new DeviceInfo { Model = "Oppo Fastboot", Mode = "Fastboot Mode", Serial = fb.SerialNumber };
            ...
            else if (adbDevices.Any())
            {
                ...
                {
                    _detectedDeviceInfo = new DeviceInfo { Model = "Oppo Recovery", Mode = "Recovery Mode", Serial = adb.SerialNumber };
                    Log($"Found Recovery Device: {adb.SerialNumber}");
                }
                else if (adb.DeviceState == "sideload")
                {
                    _detectedDeviceInfo = new DeviceInfo { Model = "Oppo Sideload", Mode = "Sideload Mode", Serial = adb.SerialNumber };
        ...
        private async Task ReadInfoAsync()
        {
            ...
            Progress = 20;
            Log("Reading Oppo Device Information...");
            ...
            else
            {
                ...
                _notificationService.ShowNotification("Oppo", "Please detect device first.", NotificationType.Warning);
            }
        ...
        private async Task FlashOfpAsync()
        {
            IsBusy = true;
            StatusText = "Flashing Fastboot ROM...";
            Log("Starting Oppo OFP Firmware Flash...");
            ...
            if (string.IsNullOrEmpty(OfpPath))
            {
            ...
            Progress = 10;
            Log($"Analyzing ROM package at: {OfpPath}...");
            await Task.Delay(1000);
