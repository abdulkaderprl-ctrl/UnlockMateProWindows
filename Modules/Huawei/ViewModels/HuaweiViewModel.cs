...
{
    public class HuaweiViewModel : ViewModelBase
    {
        ...
        public HuaweiViewModel(IAdbService adbService, IFastbootService fastbootService, ILoggerService logger, INotificationService notificationService)
        {
            ...
            BrowseFirmwareCommand = new RelayCommand(() => FirmwarePath = OpenFileDialog("Huawei Firmware (*.zip;*.tar;*.sh;*.bat)|*.zip;*.tar;*.sh;*.bat|All Files (*.*)|*.*|All Files (*.*)|*.*"));
            BrowsePersistCommand = new RelayCommand(() => PersistPath = OpenFileDialog("Persist Image (*.img)|*.img"));

            Log("Huawei Professional Module Initialized.");
        }
        ...
        private void Log(string message)
        {
            ...
            _logger.LogInfo($"[Huawei] {message}");
        }

        private async Task DetectDeviceAsync()
        {
            IsBusy = true;
            StatusText = "Detecting Huawei device...";
            Progress = 30;
            ...
            if (fastbootDevices.Any())
            {
                ...
                _detectedDeviceInfo = new DeviceInfo { Model = "Huawei Fastboot", Mode = "Fastboot Mode", Serial = fb.SerialNumber };
            ...
            else if (adbDevices.Any())
            {
                ...
                {
                    _detectedDeviceInfo = new DeviceInfo { Model = "Huawei Recovery", Mode = "Recovery Mode", Serial = adb.SerialNumber };
                    Log($"Found Recovery Device: {adb.SerialNumber}");
                }
                else if (adb.DeviceState == "sideload")
                {
                    _detectedDeviceInfo = new DeviceInfo { Model = "Huawei Sideload", Mode = "Sideload Mode", Serial = adb.SerialNumber };
        ...
        private async Task ReadInfoAsync()
        {
            ...
            Progress = 20;
            Log("Reading Huawei Device Information...");
            ...
            else
            {
                ...
                _notificationService.ShowNotification("Huawei", "Please detect device first.", NotificationType.Warning);
            }
        ...
        private async Task FlashFirmwareAsync()
        {
            ...
            Log("Starting Huawei Huawei Firmware Flash...");
