...
{
    public class MotorolaViewModel : ViewModelBase
    {
        ...
        public MotorolaViewModel(IAdbService adbService, IFastbootService fastbootService, ILoggerService logger, INotificationService notificationService)
        {
            ...
            BrowseFirmwareCommand = new RelayCommand(() => FirmwarePath = OpenFileDialog("Motorola Firmware (*.zip;*.tar;*.sh;*.bat)|*.zip;*.tar;*.sh;*.bat|All Files (*.*)|*.*|All Files (*.*)|*.*"));
            BrowsePersistCommand = new RelayCommand(() => PersistPath = OpenFileDialog("Persist Image (*.img)|*.img"));

            Log("Motorola Professional Module Initialized.");
        }
        ...
        private void Log(string message)
        {
            ...
            _logger.LogInfo($"[Motorola] {message}");
        }

        private async Task DetectDeviceAsync()
        {
            IsBusy = true;
            StatusText = "Detecting Motorola device...";
            Progress = 30;
            ...
            if (fastbootDevices.Any())
            {
                ...
                _detectedDeviceInfo = new DeviceInfo { Model = "Motorola Fastboot", Mode = "Fastboot Mode", Serial = fb.SerialNumber };
            ...
            else if (adbDevices.Any())
            {
                ...
                {
                    _detectedDeviceInfo = new DeviceInfo { Model = "Motorola Recovery", Mode = "Recovery Mode", Serial = adb.SerialNumber };
                    Log($"Found Recovery Device: {adb.SerialNumber}");
                }
                else if (adb.DeviceState == "sideload")
                {
                    _detectedDeviceInfo = new DeviceInfo { Model = "Motorola Sideload", Mode = "Sideload Mode", Serial = adb.SerialNumber };
        ...
        private async Task ReadInfoAsync()
        {
            ...
            Progress = 20;
            Log("Reading Motorola Device Information...");
            ...
            else
            {
                ...
                _notificationService.ShowNotification("Motorola", "Please detect device first.", NotificationType.Warning);
            }
        ...
        private async Task FlashFirmwareAsync()
        {
            ...
            Log("Starting Motorola Motorola Firmware Flash...");
