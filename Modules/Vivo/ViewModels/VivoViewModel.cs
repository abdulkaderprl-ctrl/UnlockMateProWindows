...
{
    public class VivoViewModel : ViewModelBase
    {
        ...
        public VivoViewModel(IAdbService adbService, IFastbootService fastbootService, ILoggerService logger, INotificationService notificationService)
        {
            ...
            BrowseFirmwareCommand = new RelayCommand(() => FirmwarePath = OpenFileDialog("Vivo Firmware (*.zip;*.tar;*.sh;*.bat)|*.zip;*.tar;*.sh;*.bat|All Files (*.*)|*.*|All Files (*.*)|*.*"));
            BrowsePersistCommand = new RelayCommand(() => PersistPath = OpenFileDialog("Persist Image (*.img)|*.img"));

            Log("Vivo Professional Module Initialized.");
        }
        ...
        private void Log(string message)
        {
            ...
            _logger.LogInfo($"[Vivo] {message}");
        }

        private async Task DetectDeviceAsync()
        {
            IsBusy = true;
            StatusText = "Detecting Vivo device...";
            Progress = 30;
            ...
            if (fastbootDevices.Any())
            {
                ...
                _detectedDeviceInfo = new DeviceInfo { Model = "Vivo Fastboot", Mode = "Fastboot Mode", Serial = fb.SerialNumber };
            ...
            else if (adbDevices.Any())
            {
                ...
                {
                    _detectedDeviceInfo = new DeviceInfo { Model = "Vivo Recovery", Mode = "Recovery Mode", Serial = adb.SerialNumber };
                    Log($"Found Recovery Device: {adb.SerialNumber}");
                }
                else if (adb.DeviceState == "sideload")
                {
                    _detectedDeviceInfo = new DeviceInfo { Model = "Vivo Sideload", Mode = "Sideload Mode", Serial = adb.SerialNumber };
        ...
        private async Task ReadInfoAsync()
        {
            ...
            Progress = 20;
            Log("Reading Vivo Device Information...");
            ...
            else
            {
                ...
                _notificationService.ShowNotification("Vivo", "Please detect device first.", NotificationType.Warning);
            }
        ...
        private async Task FlashFirmwareAsync()
        {
            ...
            Log("Starting Vivo Vivo Firmware Flash...");
