...
namespace UnlockMatePro.Services
{
    public class FastbootService : IFastbootService
    {
        ...
        public async Task<(bool Success, string Output)> ExecuteFastbootCommandAsync(string arguments, string? serialNumber = null, System.Threading.CancellationToken cancellationToken = default)
        {
            if (!_isAvailable)
            {
