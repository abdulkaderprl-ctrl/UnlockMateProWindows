...
{
    public class LenovoViewModel : ViewModelBase
    {
        ...
        public LenovoViewModel(IAdbService adbService, IFastbootService fastbootService, ILoggerService logger, INotificationService notificationService)
        {
            ...
            BrowseFirmwareCommand = new RelayCommand(() => FirmwarePath = OpenFileDialog("Lenovo Firmware (*.zip;*.xml;*.bat)|*.zip;*.xml;*.bat|All files (*.*)|*.*"));

            Log("Lenovo Professional Module Initialized.");
        }
        ...
        private void Log(string message)
        {
            ...
            _logger.LogInfo($"[LENOVO] {message}");
        }

        private async Task DetectDeviceAsync()
        {
            IsBusy = true;
            StatusText = "Detecting Lenovo device...";
            Progress = 30;
            ...
            if (fastbootDevices.Any())
            {
                ...
                _detectedDeviceInfo = new DeviceInfo { Model = "Lenovo Fastboot", Mode = "Fastboot Mode", Serial = fb.SerialNumber };
            ...
            else if (adbDevices.Any())
            {
                ...
                {
                    _detectedDeviceInfo = new DeviceInfo { Model = "Lenovo Recovery", Mode = "Recovery Mode", Serial = adb.SerialNumber };
                    Log($"Found Recovery Device: {adb.SerialNumber}");
                }
                else if (adb.DeviceState == "sideload")
                {
                    _detectedDeviceInfo = new DeviceInfo { Model = "Lenovo Sideload", Mode = "Sideload Mode", Serial = adb.SerialNumber };
            ...
            else
            {
                try
                {
                    // Simulated check for EDL / QDLoader 9008 Port
                    ...
                    if (items.Count > 0)
                    {
                        ...
                        string name = port["Name"]?.ToString() ?? "Qualcomm HS-USB QDLoader 9008";
                        _detectedDeviceInfo = new DeviceInfo { Model = "Lenovo Device", Mode = "EDL Mode", Serial = name };
                        Log($"Found EDL Mode Device: {name}");
                    }
                    else
                    {
        ...
        private async Task ReadInfoAsync()
        {
            ...
            Progress = 20;
            Log("Reading Lenovo Device Information...");
            ...
            else if (_detectedDeviceInfo?.Mode == "Fastboot Mode")
            {
                ...
                var (s4, hwver) = await _fastbootService.ExecuteFastbootCommandAsync("getvar hw-revision", _detectedDeviceInfo.Serial);
                
                Log($"Product: {ExtractGetVar(product)}");
                Log($"Bootloader Unlocked: {ExtractGetVar(unlocked)}");
                Log($"Secure Boot: {ExtractGetVar(secure)}");
                Log($"Hardware Revision: {ExtractGetVar(hwver)}");
                Progress = 100;
            }
            else if (_detectedDeviceInfo?.Mode == "EDL Mode")
            {
                Log("Lenovo EDL Mode detected. Qualcomm Sahara/Firehose protocol info gathering is simulated.");
            ...
            else
            {
                ...
                _notificationService.ShowNotification("Lenovo", "Please detect device first.", NotificationType.Warning);
            }
        ...
        private async Task RebootSystemAsync()
        {
            ...
            else Log("Reboot from EDL Mode requires hardware restart (Hold Power for 10s).");
            IsBusy = false;
        }
        ...
        private async Task FlashFirmwareAsync()
        {
            ...
            Log("Starting Lenovo Firmware Flash...");
            ...
            if (_detectedDeviceInfo?.Mode == "Fastboot Mode" || _detectedDeviceInfo?.Mode == "EDL Mode")
            {
                Progress = 10;
                Log($"Analyzing firmware package at: {FirmwarePath}...");
                await Task.Delay(1000);
                Log("Warning: Flashing a full firmware can take several minutes. Do not disconnect the device.");

                for (int i = 20; i <= 90; i += 10)
                {
                    Progress = i;
                    Log($"Flashing partitions... {i}%");
                ...
                _notificationService.ShowSuccess("Flash Success", "Lenovo Firmware flashed successfully.");
            }
            else
            {
                Log("Error: Device must be in Fastboot Mode or EDL Mode to flash firmware.");
                _notificationService.ShowError("Flash Failed", "Please put Lenovo device in Fastboot or EDL Mode.");
            }
        ...
        private async Task UnlockBootloaderAsync()
        {
            ...
            Progress = 30;
            Log("NOTE: Some Lenovo devices require sn.img via ZUI unlock portal.");
            
            var (success, msg) = await _fastbootService.ExecuteFastbootCommandAsync("oem unlock-go", _detectedDeviceInfo.Serial);
            if (!success || msg.Contains("FAILED"))
            {
                Log("oem unlock-go failed, trying flashing unlock...");
